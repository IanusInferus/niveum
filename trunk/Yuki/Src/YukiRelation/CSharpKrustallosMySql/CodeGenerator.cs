...
namespace Yuki.RelationSchema.CSharpKrustallosMySql
{
    public static class CodeGenerator
    {
        ...
        {
            var w = new Writer(Schema, EntityNamespaceName, KrustallosContextNamespaceName, MySqlContextNamespaceName, ContextNamespaceName);
            var a = w.GetSchema();
            return String.Join("\r\n", a);
        }
        ...
        public class Writer
        {
            ...
            public List<String> GetSchema()
            {
                ...
                if (NamespaceName != "")
                {
                    return EvaluateEscapedIdentifiers(GetTemplate("MainWithNamespace").Substitute("Header", Header).Substitute("NamespaceName", NamespaceName).Substitute("Imports", Imports).Substitute("Primitives", Primitives).Substitute("ComplexTypes", ComplexTypes)).Select(Line => Line.TrimEnd(' ')).ToList();
                }
                else
                {
