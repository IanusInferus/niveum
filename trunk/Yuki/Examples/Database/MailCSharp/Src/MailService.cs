    ...
    public class MailService
    {
        ...
        public MailDetail GetMail(int MailId)
        {
            using (var da = dam.Create())
            {
                ...
                var Attachments = da.SelectManyMailAttachmentForNameById(m.Id).ToList();
        ...
        public void SendMail(MailInput m)
        {
            using (var da = dam.Create())
            {
                ...
                da.InsertManyMailAttachment(m.Attachments.Select(ma => new DB.MailAttachment { Id = Id, Name = ma.Name, Content = ma.Content }).ToList());
                da.Complete();
            }
        }
    }
