        ...
        public class Writer
        {
            ...
            public List<String> GetJsonTranslatorSerializers()
            {
                ...
                foreach (var c in Schema.TypeRefs.Concat(Schema.Types))
                {
                    ...
                    {
                        l.AddRange(GetJsonTranslatorRecord(new RecordDef { Name = c.Alias.Name, Version = c.Alias.Version, GenericParameters = c.Alias.GenericParameters, Fields = new List<VariableDef> { new VariableDef { Name = "Value", Type = c.Alias.Type, Attributes = new List<KeyValuePair<String, List<String>>> { }, Description = "" } }, Attributes = new List<KeyValuePair<String, List<String>>> { }, Description = "" }));
                    }
                    else if (c.OnRecord)
                    {
                ...
                {
                    l.AddRange(GetJsonTranslatorRecord(new RecordDef { Name = t.TypeFriendlyName(), Version = "", GenericParameters = new List<VariableDef> { }, Fields = t.Tuple.Select((tp, i) => new VariableDef { Name = String.Format("Item{0}", i), Type = tp, Description = "" }).ToList(), Attributes = new List<KeyValuePair<String, List<String>>> { }, Description = "" }));
                    l.Add("");
                }
                ...
                foreach (var gts in GenericTypeSpecs)
                {
                    if (gts.GenericTypeSpec.TypeSpec.OnTypeRef && gts.GenericTypeSpec.TypeSpec.TypeRef.Name == "Optional" && gts.GenericTypeSpec.ParameterValues.Count == 1)
                    {
                        ...
                        l.AddRange(GetJsonTranslatorTaggedUnion(new TaggedUnionDef { Name = Name, Version = "", GenericParameters = new List<VariableDef> { }, Alternatives = Alternatives, Attributes = GenericOptionalType.Attributes, Description = GenericOptionalType.Description }));
                    }
            ...
            public List<String> GetJsonTranslatorMap(TypeSpec c)
            {
                ...
                var ValueTypeFriendlyName = c.GenericTypeSpec.ParameterValues[0].TypeFriendlyName();
