...
namespace Niveum.RelationSchema
{
    ...
    public sealed class RelationSchemaLoader
    {
        ...
        private VariableDef FillFieldAttribute(VariableDef f, Dictionary<String, EntityDef> Entities)
        {
            var t = f.Type;
            var IsColumn = false;
            if (t.OnTypeRef)
            ...
            var FinalType = f.Type;
            if (IsNullable && !t.OnOptional)
            {
                FinalType = TypeSpec.CreateOptional(t.TypeRef);
                if (Positions.ContainsKey(t))
                {
                    Positions.Add(FinalType, Positions[t]);
                }
            }
        ...
        private EntityDef FillEntity(EntityDef e, Dictionary<String, EntityDef> Entities)
        {
            var Fields = new List<VariableDef>();
            for (int i = 0; i < e.Fields.Count; i++)
            {
                var f = FillFieldAttribute(e.Fields[i], Entities);
                Fields.Add(f);
            ...
            if (PrimaryKey == null)
            {
                var Ids = Fields.Where(f => f.Name.Equals("Id", StringComparison.Ordinal) || f.Name.Equals(e.Name + "Id", StringComparison.Ordinal)).ToList();
                if (Ids.Count > 1)
                {
            ...
            {
                PrimaryKey = new Key { Columns = PrimaryKey.Columns, IsClustered = true };
            }
        ...
        private EntityDef FillEntityNavigations(EntityDef e, Dictionary<String, EntityDef> Entities)
        {
            //如果一个非简单类型属性(导航属性)没有标明外键或反外键，则
            //    1)如果有<Name>Id的列，且该列为简单类型，类型表的主键列数量为1，则将该字段标明为[FK:<Name>Id=<Type/ElementType>.<PrimaryKey>]
            //    2)如果类型表有一个<TableName>Id的列，且该列为简单类型，当前表的主键列数量为1，则将该字段标明为[RFK:<PrimaryKey>=<Type/ElementType>.<TableName>Id]
            ...
            for (int i = 0; i < e.Fields.Count; i++)
            {
                ...
                if (f.Attribute.OnNavigation)
                {
                    ...
                    {
                        var FieldNameIds = Fields.Where(rf => rf.Attribute.OnColumn && rf.Name.Equals(f.Name + "Id", StringComparison.OrdinalIgnoreCase)).ToList();
                        if (FieldNameIds.Count == 1)
                        {
                            ...
                            if (ExternalEntity.PrimaryKey.Columns.Count == 1)
                            {
                                f = new VariableDef
                                {
                                    ...
                                    Attributes = f.Attributes,
                                    Description = f.Description,
                                    Attribute = FieldAttribute.CreateNavigation(new NavigationAttribute { IsReverse = false, IsUnique = true, ThisKey = new List<String> { FieldNameId.Name }, OtherKey = new List<String> { ExternalEntity.PrimaryKey.Columns.Select(c => c.Name).Single() } })
                                };
                                Fields.Add(f);
                                continue;
                            }
                        ...
                        if (TableNameIds.Count == 1)
                        {
                            ...
                            if (e.PrimaryKey.Columns.Count == 1)
                            {
                                f = new VariableDef
                                {
                                    ...
                                    Attributes = f.Attributes,
                                    Description = f.Description,
                                    Attribute = FieldAttribute.CreateNavigation(new NavigationAttribute { IsReverse = true, IsUnique = true, ThisKey = new List<String> { e.PrimaryKey.Columns.Select(c => c.Name).Single() }, OtherKey = new List<String> { TableNameId.Name } })
                                };
                                Fields.Add(f);
                                continue;
                            }
