    ...
    partial class Templates
    {
        ...
        public IEnumerable<String> Type(TypeDef t)
        {
            ...
            else if (t.OnEntity)
            {
                ...
                yield return "<table>";
                foreach (var _Line in Combine(Combine(Begin(), "    "), Variables(t.Entity.Fields)))
                {
                    yield return _Line;
                }
                yield return "</table>";
        ...
        public IEnumerable<String> Variable(String Name, String TypeSpec, String Description)
        {
            yield return "<tr>";
            foreach (var _Line in Combine(Combine(Combine(Begin(), "    <td>"), GetEscaped(Name)), "</td>"))
            {
            ...
            foreach (var _Line in Combine(Combine(Combine(Begin(), "    <td>"), TypeSpec), "</td>"))
            {
                yield return _Line;
            }
        ...
        public IEnumerable<String> Variables(List<VariableDef> l)
        {
            if (l.Count > 0)
            {
                foreach (var v in l)
                {
                    var TypeSpecString = GetFieldTypeString(v);
                    foreach (var _Line in Combine(Begin(), Variable(v.Name, TypeSpecString, v.Description)))
                    {
                        yield return _Line;
                    }
        ...
        {
            var ts = TypeSpec.CreateTypeRef(new TypeRef { Value = t.Name() });
            var MetaType = GetMetaType(t);
            var Description = t.Description();
            yield return "<tr>";
