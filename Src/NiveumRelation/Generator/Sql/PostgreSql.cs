...
namespace Niveum.RelationSchema.PostgreSql
{
    ...
    public partial class Templates
    {
        ...
        public List<String> GetSchema()
        {
            ...
            return Main(Lc(DatabaseName), Tables, ForeignKeys, Comments).Select(Line => Line.TrimEnd(' ')).ToList();
        }
        ...
        {
            return Key("PRIMARY KEY", Lc(Name), GetClusterKeyword(k.IsClustered), GetColumns(k.Columns));
        }

        public IEnumerable<String> GetUniqueKey(Key k, String Name)
        {
            return Key("UNIQUE", Lc(Name), GetClusterKeyword(k.IsClustered), GetColumns(k.Columns));
        }

        public IEnumerable<String> GetNonUniqueKey(Key k, String Name, String TableName)
        {
            return NonUniqueKey(Lc(Name), Lc(TableName), GetClusterKeyword(k.IsClustered), GetColumns(k.Columns));
        }
        ...
        {
            return ForeignKey(Lc(Name), Lc(ThisTableName), GetForeignColumns(ThisKeyColumns), Lc(OtherTableName), GetForeignColumns(OtherKeyColumns));
        }
        ...
        public IEnumerable<String> GetComments(Schema s, Boolean WithComment)
        {
            if (!WithComment) { return new List<String> { }; }
            var l = new List<String>();
            foreach (var t in s.Types.Where(Type => Type.OnEntity).Select(Type => Type.Entity))
            {
                l.AddRange(TableComment(Lc(t.CollectionName), GetSqlStringLiteral(t.Description)));
                foreach (var c in t.Fields.Where(Field => Field.Attribute.OnColumn))
                {
                    l.AddRange(ColumnComment(Lc(t.CollectionName), Lc(c.Name), GetSqlStringLiteral(c.Description)));
                }
            }
            return l;
        }
