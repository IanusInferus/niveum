...
namespace Niveum.RelationSchema.TSql
{
    ...
    public partial class Templates
    {
        ...
        public List<String> GetSchema()
        {
            ...
            return Main(DatabaseName, Tables, ForeignKeys, Comments).Select(Line => Line.TrimEnd(' ')).ToList();
        }
        ...
        public IEnumerable<String> GetTable(EntityDef r)
        {
            ...
            return Table(r.CollectionName, JoinWithComma(FieldsAndKeys.ToArray()), NonUniqueKeys).ToList();
        }
        ...
        {
            return JoinWithComma(Columns.Select(c => new List<String> { c.IsDescending ? String.Format("[{0}] DESC", c.Name) : String.Format("[{0}]", c.Name) }).ToArray());
        }
        ...
        public IEnumerable<String> GetComments(Schema s, Boolean WithComment)
        {
            ...
            {
                l.AddRange(TableComment(t.CollectionName, GetSqlStringLiteral(t.Description)));
                foreach (var c in t.Fields.Where(Field => Field.Attribute.OnColumn))
                {
                    l.AddRange(ColumnComment(t.CollectionName, c.Name, GetSqlStringLiteral(c.Description)));
                }
            }
            return l;
        }
