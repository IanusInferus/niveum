...
namespace Yuki.SchemaManipulator
{
    public static class Program
    {
        ...
        public static int MainInner()
        {
            ...
            foreach (var opt in CmdLine.Options)
            {
                ...
                else if (optNameLower == "gencom")
                {
                    ...
                    if (args.Length == 5)
                    {
                        GenerateCompatibilityTreeFile(args[0], args[1], args[2], args[3], args[4]);
                    }
                    else
                    {
                        DisplayInfo();
        ...
        public static void GenerateCompatibilityTreeFile(String OldCookedObjectSchemaFile, String OldVersion, String NewCookedObjectSchemaFile, String NewVersion, String CompatibilityObjectSchemaFile)
        {
            ...
            var Comment = ""
                ...
                + @"" + "\r\n"
                + @"  SchemaHash: Head 0x" + HeadCommandSchema.Hash().ToString("X16") + "\r\n"
                + @"              " + NewVersion + " 0x" + NewCommandSchema.Hash().ToString("X16") + "\r\n"
                + @"              " + OldVersion + " 0x" + OldCommandSchema.Hash().ToString("X16") + "\r\n"
                + @"" + "\r\n"
                + @"==========================================================================" + "\r\n"
                ;
            ...
            var Patch = DiffGenerator.Generate(HeadObjectSchema, OldCommandSchema.GetSubSchema(OldCommandSchema.Types.Where(t => ChangedCommands.Contains(t.VersionedName())).ToArray(), new TypeSpec[] { })).Patch;
